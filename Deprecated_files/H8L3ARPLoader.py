...
# 文件日志处理器
file_handler = logging.FileHandler('h8l3arp_download.log')
...
def process_l3arp_file_to_nc(l3arp_file_path, lookup_df, output_path):
    """处理L3ARP文件并保存为.nc数据集"""
    try:
        # 创建输出文件
        with nc.Dataset(output_path, 'w', format='NETCDF4') as ds_out:
            ...
            # 打开输入文件
            with nc.Dataset(l3arp_file_path, 'r') as dataset:
                qa_flag_var = dataset.variables['QA_flag_Merged']
                aot_var_in = dataset.variables['AOT_Merged']
                aot_uncertainty_var_in = dataset.variables['AOT_Merged_uncertainty']
...
def download_and_process(date, hour, lookup_df, base_dir):
    """下载并处理单个L3ARP文件（小时分辨率）"""
    global vacant_file_path

    # 创建月份文件夹
    month_dir = os.path.join(base_dir, "H8L3ARP", f"{date.year:04d}", f"{date.month:02d}")
    os.makedirs(month_dir, exist_ok=True)

    # 确定卫星标识（2022年12月13日0000开始使用H09）
    threshold_date = datetime.datetime(2022, 12, 13, 0, 0)
    current_dt = datetime.datetime(date.year, date.month, date.day, hour)

    if current_dt >= threshold_date:
        satellite_id = "H09"
    else:
        satellite_id = "H08"

    # 设置文件名和路径
    ftp_path = f"/pub/himawari/L3/ARP/031/{date.year:04d}{date.month:02d}/{date.day:02d}/{satellite_id}_{date.year:04d}{date.month:02d}{date.day:02d}_{hour:02d}00_1HARP031_FLDK.02401_02401.nc"
    local_filename = f"{satellite_id.lower()}_{date.strftime('%Y%m%d')}_{hour:02d}00.nc"  # 更新本地文件名
    local_filepath = os.path.join(month_dir, local_filename)

    # 输出小文件名（保持为H8L3ARP，因为产品名称不变）
    small_nc_filename = f"H8L3ARP_{date.year:04d}{date.month:02d}{date.day:02d}_{hour:02d}00.nc"
    ...
    # 如果文件下载成功，则处理文件
    if downloaded_file:
        success = process_l3arp_file_to_nc(downloaded_file, lookup_df, small_nc_path)
        if success:
...
def integrate_monthly_data(month_dir, year, month):
    ...
    # 查找所有小文件
    small_files = glob.glob(os.path.join(month_dir, "H8L3ARP_*.nc"))
    if not small_files:
    ...
    # 月度数据集输出路径
    output_path = os.path.join(month_dir, f"H8L3ARP_monthly_{year:04d}{month:02d}.nc")
    ...
    with nc.Dataset(output_path, 'w', format='NETCDF4') as ds_out:
        ...
        for i, file_path in enumerate(small_files):
            ...
            try:
                ...
                # 文件名格式: H8L3ARP_YYYYMMDD_HH00.nc
                ...
                time_str = parts[1] + parts[2].split('.')[0]
...
def main():
    ...
    # 缺失文件记录路径（改为H8L3ARP目录）
    h8l3arp_base_dir = os.path.join(Data_path, "H8L3ARP")
    os.makedirs(h8l3arp_base_dir, exist_ok=True)
    vacant_file_path = os.path.join(h8l3arp_base_dir, "H8L3ARP_vacant.csv")
    ...
    while current_date <= end_date:
        ...
        # 创建月份文件夹
        month_dir = os.path.join(h8l3arp_base_dir, f"{year:04d}", f"{month:02d}")
        os.makedirs(month_dir, exist_ok=True)

        logger.info(f"开始处理 {year:04d}-{month:02d} 的H8L3ARP数据")
