...
PATHS = {
    ...
    "modis_brdf": "D:/H8_data/MODIS_BRDFparams" ,
...
def load_daily_brdf_data(date):
    ...
    year = date_str[:4]
    month = date_str[4:6]
    file_path = os.path.join(PATHS["modis_brdf"], year, month, f"MODIS_BRDFparams_{date_str}.nc")

    if not os.path.exists(file_path):
        print(f"[{timestamp()}] [WARNING] MODIS BRDF file not found: {file_path}")
        return {}
...
def process_station_worker(task):
    """工作进程：处理单个站点并记录详细信息"""
    station, idx, hourly, merra2_slv, merra2_aer, lucc_dict, coords, date, hour, result_idx, brdf_params = task
    lat, lon = coords.get(station, (np.nan, np.nan))

    # 初始化结果和记录字典
    sr_6ssr = np.full(len(BAND_WAVELENGTHS), np.nan, dtype=np.float32)
    ...
    try:
        ...
        if hourly.get('hourly_availability', [1])[idx] != 0:
            ...
            return (result_idx, gen_avail, sr_6ssr, sr_h8sr, valid_flag, record)
        ...
        record['LUCC'] = lucc_value
        record['brdf_model'] = LUCC_TO_BRDF_MODEL.get(int(lucc_value), 'Lambertian')
        ...
        try:
            ...
            s_6s.geometry.solar_z = record['SOZ']
            s_6s.geometry.solar_a = record['SOA']
            s_6s.geometry.view_z = record['SAZ']
            s_6s.geometry.view_a = record['SAA']
            ...
            for i, (wvl, refl) in enumerate(zip(BAND_WAVELENGTHS, toa_refs)):
                ...
                record[f'SR_6SSR_{BAND_NAMES[i]}'] = sr_6ssr[i]
        finally:
            del s_6s
            gc.collect()
        ...
        try:
            ...
            for i, (wvl, refl) in enumerate(zip(BAND_WAVELENGTHS, toa_refs)):
                ...
                # 检查参数有效性
                if not any(np.isnan(p) for p in [iso, vol, geo]):
                    # 使用MODIS BRDF参数
                    s_brdf.ground_reflectance = GroundReflectance.HomogeneousMODISBRDF(iso, vol, geo)
                    record[f'BRDF_source_{BAND_NAMES[i]}'] = 'MODIS'
                else:
                    # 回退到LUCC映射模型
                    s_brdf = set_brdf_model(s_brdf, lucc_value)
                    record[f'BRDF_source_{BAND_NAMES[i]}'] = 'LUCC_Fallback'
                ...
                try:
                    ...
                    record[f'SR_H8SR_{BAND_NAMES[i]}'] = np.nan
                    record['error'] = f'BRDF band {i} failed: {str(e)}'
    ...
    try:
        ...
        for station in matched:
            ...
            tasks.append((
                ...
                lucc_dict, station_coords, date, hour, sidx,
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    result_idx, gen_avail, sr_6ssr, sr_h8sr, valid_flag, record = future.result()
