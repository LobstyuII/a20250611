...
def save_daily_data(date, mod13a1_data, stations, lats, lons, grid_info, base_path):
    """保存单日数据到NC文件（只包含MOD13A1数据）"""
    try:
        # 创建年月目录
        year = date.strftime("%Y")
        month = date.strftime("%m")
        dir_path = os.path.join(base_path, "MODIS_NDVI_MOD13A1", year, month)
        os.makedirs(dir_path, exist_ok=True)

        # 文件路径
        filename = f"MOD13A1_NDVI_{date.strftime('%Y%m%d')}.nc"
        ...
        with nc.Dataset(file_path, 'w', format='NETCDF4') as ds:
            ...
            ndvi_var = ds.createVariable('NDVI_MOD13A1', 'f4', ('Station',), fill_value=-9999.0)
...
def calculate_composite_date(target_date):
    """计算目标日期对应的16天合成期的中心日期"""
    # MOD13A1合成期规则：每年1月1日开始，每16天一个周期
    year_start = datetime.date(target_date.year, 1, 1)
    day_of_year = (target_date - year_start).days + 1

    # 计算合成期编号 (0-based)
    period_index = (day_of_year - 1) // 16

    # 计算合成期中心日期的年积日 (DOY)
    center_doy = period_index * 16 + 8 + 1  # 加1因为1月1日是DOY=1

    # 转换为实际日期
    center_date = year_start + datetime.timedelta(days=center_doy - 1)
    return center_date


def process_daily_data(date, mod13a1_col, grid_fc, stations, lats, lons, grid_info, base_path, max_retries=5):
    """处理单日数据（只使用MOD13A1）"""
    retry_count = 0
    while retry_count < max_retries:
        try:
            # 计算该日期对应的16天合成期的中心日期
            center_date = calculate_composite_date(date)
            start_date = center_date - datetime.timedelta(days=8)
            end_date = center_date + datetime.timedelta(days=8)
            ...
            # 获取MOD13A1数据（使用区域统计）
            mod13a1_img = mod13a1_col.filterDate(start_date_str, end_date_str).first()
            mod13a1_data = [-9999.0] * len(stations)

            if mod13a1_img:
                # 应用缩放因子 (0.0001)
                scaled_img = mod13a1_img.multiply(0.0001)

                mod13a1_reduced = scaled_img.reduceRegions(
                    collection=grid_fc,
                    reducer=ee.Reducer.mean(),
                    scale=500  # MODIS原始分辨率
                )
                mod13a1_info = mod13a1_reduced.getInfo()

                if 'features' in mod13a1_info:
                    # 构建站点到NDVI的映射
                    temp_data = {}
                    for f in mod13a1_info['features']:
                        props = f['properties']
                    ...
                    mod13a1_data = [temp_data.get(station, -9999.0) for station in stations]

            # 保存数据（只传递MOD13A1数据）
            return save_daily_data(date, mod13a1_data, stations, lats, lons, grid_info, base_path)
...
def main():
    ...
    # 定义GEE ImageCollection (使用MOD13A1版本061)
    mod13a1_col = ee.ImageCollection("MODIS/061/MOD13A1").select('NDVI')
    logging.info("已初始化MOD13A1 NDVI集合 (版本061)")
    ...
    for date in unique_dates:
        ...
        month = date.strftime("%m")
        filename = f"MOD13A1_NDVI_{date.strftime('%Y%m%d')}.nc"
        file_path = os.path.join(data_path, "MODIS_NDVI_MOD13A1", year, month, filename)
    ...
    with tqdm(total=total_tasks, desc="下载进度") as pbar:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            futures = {executor.submit(
                process_daily_data,
                date, mod13a1_col, grid_fc, stations, lats, lons, grid_info, data_path
...
def merge_modis_data(base_path, output_path):
    ...
    try:
        # 收集所有文件路径
        all_files = []
        modis_dir = os.path.join(base_path, "MODIS_NDVI_MOD13A1")

        for root, dirs, files in os.walk(modis_dir):
            for file in files:
                if file.endswith(".nc") and file.startswith("MOD13A1_NDVI_"):
                    all_files.append(os.path.join(root, file))

        if not all_files:
            logging.error("未找到MOD13A1 NDVI数据文件")
            return
...
if __name__ == "__main__":
    ...
    # 第二步：合并数据（下载完成后运行）
    # data_path = "D:/H8_data"
    # output_nc = os.path.join(data_path, "MOD13A1_NDVI_combined.nc")
