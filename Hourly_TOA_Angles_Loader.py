...
OUTPUT_DIR = r'D:\H8_data\Hourly_TOA_Angles'
STATION_COUNT = 2014
...
def get_station_names():
    ...
    if not sample_file:
        raise FileNotFoundError(f"找不到L1TOA文件来获取站点名称。请检查路径: {L1TOA_DIR}")

    logger.info(f"使用示例文件获取站点名称: {sample_file}")
...
def validate_file_path(year, month, day, hour, minute):
    ...
    # 尝试不同的文件路径模式
    l1toa_patterns = [
        os.path.join(L1TOA_DIR, f"{year}", f"{month:02d}", f"H8_{date_str}_{hour:02d}{minute:02d}.nc"),
        os.path.join(L1TOA_DIR, f"{year}", f"{month}", f"H8_{date_str}_{hour:02d}{minute:02d}.nc"),
        os.path.join(L1TOA_DIR, f"{year}", f"{month:02d}", f"{day:02d}", f"H8_{date_str}_{hour:02d}{minute:02d}.nc")
    ]

    l2arp_patterns = [
        os.path.join(L2ARP_DIR, f"{year}", f"{month:02d}", f"H8L2ARP_{date_str}_{hour:02d}{minute:02d}.nc"),
        os.path.join(L2ARP_DIR, f"{year}", f"{month}", f"H8L2ARP_{date_str}_{hour:02d}{minute:02d}.nc"),
        os.path.join(L2ARP_DIR, f"{year}", f"{month:02d}", f"{day:02d}",
                     f"H8L2ARP_{date_str}_{hour:02d}{minute:02d}.nc")
    ]

    # 查找存在的文件
    l1toa_path = None
    for pattern in l1toa_patterns:
        if os.path.exists(pattern):
            l1toa_path = pattern
            break

    l2arp_path = None
    for pattern in l2arp_patterns:
        if os.path.exists(pattern):
            l2arp_path = pattern
            break

    return l1toa_path, l2arp_path
...
def process_hourly_data(start_date, end_date):
    ...
    while current_date <= end_date:
        ...
        for hour in [*range(0, 13), *range(21, 23)]:
            ...
            output_file = os.path.join(output_path, f"H8_hourly_TOA_angles_{year}{month:02d}{day:02d}_{hour:02d}00.nc")
            ...
            try:
                with nc.Dataset(output_file, 'w', format='NETCDF4') as out_ds:
                    ...
                    out_ds.title = "Himawari-8 Hourly TOA & Angles POIs Dataset"
