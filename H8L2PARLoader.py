...
# 文件日志处理器
file_handler = logging.FileHandler('h8l2par_download.log')
...
def process_l2par_file_to_nc(l2par_file_path, lookup_df, output_path):
    """处理L2 PAR文件并保存为.nc数据集"""
    try:
        # 创建输出文件
        with nc.Dataset(output_path, 'w', format='NETCDF4') as ds_out:
            ...
            # 添加时间属性
            filename = os.path.basename(l2par_file_path)
            # 文件名格式: H08_YYYYMMDD_HHMM_*.nc
            time_str = filename.split('_')[2]  # 获取时间部分 (0000)
            ds_out.setncattr('time', time_str[:2] + ':' + time_str[2:])  # 格式化为 HH:MM

            # 打开输入文件
            with nc.Dataset(l2par_file_path, 'r') as dataset:
                # 获取PAR变量及其属性
...
def download_and_process(date, hour, minute, lookup_df, base_dir):
    ...
    # 创建月份文件夹
    month_dir = os.path.join(base_dir, "H8L2PAR", f"{date.year:04d}", f"{date.month:02d}")
    os.makedirs(month_dir, exist_ok=True)

    # 设置文件名和路径
    ftp_path = (f"/pub/himawari/L2/PAR/021/{date.year:04d}{date.month:02d}/"
                f"{date.day:02d}/{hour:02d}/"
                f"H08_{date.year:04d}{date.month:02d}{date.day:02d}_{hour:02d}{minute:02d}_RFL021_FLDK.02401_02401.nc")

    local_filename = f"PAR_{date.year:04d}{date.month:02d}{date.day:02d}_{hour:02d}{minute:02d}.nc"
    local_filepath = os.path.join(month_dir, local_filename)

    # 输出小文件名
    small_nc_filename = f"H8L2PAR_{date.year:04d}{date.month:02d}{date.day:02d}_{hour:02d}{minute:02d}.nc"
    ...
    # 如果文件下载成功，则处理文件
    if downloaded_file:
        success = process_l2par_file_to_nc(downloaded_file, lookup_df, small_nc_path)
        if success:
...
def integrate_monthly_data(month_dir, year, month):
    ...
    # 查找所有小文件
    small_files = glob.glob(os.path.join(month_dir, "H8L2PAR_*.nc"))
    if not small_files:
    ...
    # 月度数据集输出路径
    output_path = os.path.join(month_dir, f"H8L2PAR_monthly_{year:04d}{month:02d}.nc")
    ...
    with nc.Dataset(output_path, 'w', format='NETCDF4') as ds_out:
        ...
        for i, file_path in enumerate(small_files):
            ...
            try:
                ...
                # 文件名格式: H8L2PAR_YYYYMMDD_HHMM.nc
...
def main():
    ...
    # 缺失文件记录路径
    h8l2par_base_dir = os.path.join(Data_path, "H8L2PAR")
    os.makedirs(h8l2par_base_dir, exist_ok=True)
    vacant_file_path = os.path.join(h8l2par_base_dir, "H8L2PAR_vacant.csv")
    ...
    hours = list(range(0, 13) + range(21, 23))
    minutes = [0, 10, 20, 30, 40, 50]  # 每10分钟一次观测
    ...
    while current_date <= end_date:
        ...
        # 创建月份文件夹
        month_dir = os.path.join(h8l2par_base_dir, f"{year:04d}", f"{month:02d}")
        os.makedirs(month_dir, exist_ok=True)
